...
interface BilanData {
  ...
  notes: {
    moyenne: number;
    count: number;
    parNote: { label: string; moyenne: number | null }[];
  };
...
export function CardBilanEleve() {
  const [data, setData] = useState<BilanData | null>(null);

  useEffect(() => {
    fetch("/api/eleve/dashboard/bilan")
      .then((r) => {
  ...
  return (
    <div className="w-full max-w-[580px] bg-[#111827] rounded-2xl overflow-hidden shadow-lg">
      ...
      <div className="px-6 pt-4 pb-6 text-center">
        ...
        <p className="text-gray-400 text-sm mt-1">De l'élève</p>
      </div>
      ...
      <div className="px-6 pb-6 space-y-4">
        ...
        <div>
          <p className="text-gray-400 text-sm mb-2">Notes annuelles</p>
          <div className="flex flex-wrap gap-2">
            {data?.notes.parNote.map((n) => (
              <Badge
                key={n.label}
                className="bg-[#1f2937] text-white border border-[#374151]"
              >
                {n.label}: {n.moyenne !== null ? n.moyenne.toFixed(1) : "-"}
              </Badge>
            ))}
          </div>
        </div>
