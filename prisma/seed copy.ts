...
async function main() {
  ...
  for (const p of periods) {
    await prisma.repartition.upsert({
  ...
  for (const [levelCode, periodsData] of Object.entries(curriculaData)) {
    ...
    for (const periodData of periodsData) {
      const period = await prisma.repartition.findUniqueOrThrow({
        where: { schoolYearId_code: { schoolYearId: year.id, code: periodData.period } },
      });

      const curriculum = await prisma.curriculum.upsert({
        where: {
          schoolYearId_classLevelId_repartitionId: {
            schoolYearId: year.id,
            classLevelId: level.id,
            repartitionId: period.id,
          },
        },
        update: {
      ...
      for (let i = 0; i < periodData.modules.length; i++) {
        const [code, title, notions] = periodData.modules[i] as [string, string, string[]];
